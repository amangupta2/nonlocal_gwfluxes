...
restart=False
init_epoch=35 # which epoch to resume from. Should have restart file from init_epoch-1 ready
nepochs=35

#log_filename=f"./icml_train_ann-cnn_1x1_global_4hl_hdim-2idim_restart_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
log_filename=f"./icml_train_ann-cnn_1x1_global_4hl_threads_hdim-2idim_restart_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
def write_log(*args):
...
write_log('In this Ablation study, dropout had been switched to 0')
...
trainloader1 = torch.utils.data.DataLoader(trainset1, batch_size=bs_train, 
                                          drop_last=False, shuffle=False, num_workers=6)#, persistent_workers=True)
...
model1     = ANN_CNN(idim=idim,odim=odim,hdim=hdim,dropout=0., stencil=trainset1.stencil)
...
file_prefix = "torch_saved_models/icml_global/era5_global_ann_cnn_leakyrelu_threads_cyclic_mseloss"
#nepochs=50 #30
if restart:
    ...
    odim    = trainset1.odim
    hdim    = 2*idim
    model1     = ANN_CNN(idim=idim,odim=odim,hdim=hdim,dropout=0.1, stencil=trainset1.stencil)
    write_log(f'model1 created. \n --- model1 size: {model1.totalsize():.2f} MBs,\n --- Num params: {model1.totalparams()/10**6:.3f} mil. ')
