...
#log_filename=f"./icml_train_ann-cnn_1x1_global_4hl_hdim-2idim_restart_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
log_filename=f"./icml_train_ann-cnn_1x1_global_4hl_densityscaledrmse_hdim-2idim_restart_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
#log_filename=f"./icml_train_ann-cnn_1x1_global_4hl_nodropout_hdim-2idim_restart_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
def write_log(*args):
...
write_log('In this Ablation study, RMSE is scaled by the density to the power 0.1')
    ...
    for epoch in np.arange(init_epoch + 0, init_epoch + nepochs):
        ...
        for i, (inp, out) in enumerate(trainloader):
            ...
            if stencil==1:
                ...
                out = out.reshape(S[0]*S[1],-1)
            pred   =model(inp)
            loss     = loss_fn(pred*fac,out*fac) #loss_fn(pred,out)#loss_fn(pred*fac,out*fac) #+ weight_decay*l2_norm  #/fac) + 
        ...
        for i, (inp, out) in enumerate(testloader):
            ...
            if stencil==1:
                ...
                out = out.reshape(S[0]*S[1],-1)
            pred   =model(inp)
            loss2     = loss_fn(pred*fac,out*fac) #loss_fn(pred,out)
            testloss += loss2.item()
            count+=1
...
trainset1 = Dataset(files=train_files,domain='global', stencil=1, manual_shuffle=True, batch_size=bs_train)
trainloader1 = torch.utils.data.DataLoader(trainset1, batch_size=bs_train, 
                                          drop_last=False, shuffle=False, num_workers=0)#, persistent_workers=True)

testset1 = Dataset(files=test_files, domain='global', stencil=1, manual_shuffle=True, batch_size=bs_test)
...
rho2 = np.concatenate((rho[15:],rho[15:]),axis=0)
fac = torch.from_numpy(rho2**0.1)#torch.ones(122)#torch.from_numpy(rho[15:]**0.1)
...
#file_prefix = "torch_saved_models/icml_global/era5_global_ann_cnn_leakyrelu_dropout0p1_cyclic_mseloss" 
#file_prefix = "torch_saved_models/icml_global/era5_global_ann_cnn_leakyrelu_NOdropout_cyclic_mseloss"
file_prefix = "torch_saved_models/icml_global/era5_global_ann_cnn_leakyrelu_densityscaledrmse_cyclic_mseloss"
#nepochs=50 #30
if restart:
