...
log_filename=f"./icml_train_ann-cnn_1x1_global_4hl_withbatchnorm_hdim-2idim_restart_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
def write_log(*args):
...
write_log('In this Ablation study, batchnorm is added for all layers. The normalization done for each step now.')
...
class ANN_CNN(nn.Module):
    ...
    def __init__(self,idim, odim, hdim, stencil, dropout=0):
        ...
        self.bnorm4   = nn.BatchNorm1d(hdim)
    ...
    def forward(self, x):
        ...
        x = self.dropout5(self.bnorm5(self.act5(self.layer5(x))))
        x = self.dropout6(self.bnorm6(self.act6(self.layer6(x))))
        x = self.output(x)
...
file_prefix = "torch_saved_models/icml_global/era5_global_ann_cnn_leakyrelu_withbatchnorm_cyclic_mseloss"
#nepochs=50 #30
if restart:
