...
trainset1 = Dataset(files=train_files,domain='regional', region=rgn, stencil=3, manual_shuffle=False, batch_size=bs_train)
...
testset1 = Dataset(files=test_files, domain='regional', region=rgn, stencil=3, manual_shuffle=False, batch_size=bs_test)
testloader1 = torch.utils.data.DataLoader(testset1, batch_size=bs_test, 
                                         drop_last=False, shuffle=False, num_workers=8)#, persistent_workers=True)

tend=time2()
write_log(f'setting num_workers=8 for data loading')
...
write_log('launching cyclic schedule from 1e-5 to 1e-3 over 50 cycles')
...
file_prefix = f"torch_saved_models/regional_training/3x3_era5_regional_{sys.argv[1]}_ann_cnn_leakyrelu_dropout0p2_cyclic_mseloss" 
#nepochs=50 #30
if restart:
