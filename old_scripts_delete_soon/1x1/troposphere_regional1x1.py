# Huge validation errors in regional 1x1 ANNs due to strong winds in the upper stratosphere during winters
# Training this one only in the troposphere - does that lead to faster convergence?
import sys
import math
import numpy as np
...
log_filename=f"./icml_train_ann-cnn_1x1_regional_{sys.argv[1]}_troposphereonly_4hl_hdim-4idim_restart_epoch_{init_epoch}_to_{init_epoch+nepochs-1}.txt"
...
write_log('In this Ablation study, multiple threads are used to make batches for global training. 10 CPUs are requested and 8 CPUs are used.')
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, files, domain, stencil, batch_size, manual_shuffle, region='1andes',troposphere_only=False):
        ...
        self.troposphere_only = troposphere_only
        if troposphere_only:
            print('Troposphere Only')
            self.idim = 3 + 77*3
            self.odim = 77*2
            self.vi = np.concatenate(( np.array([0,1,2]),np.arange(48,125),np.arange(170,247),np.arange(292,369)), axis=0)
            self.vo = np.concatenate(( np.arange(45,122),np.arange(167,244)), axis=0)
    ...
    def __getitem__(self, ind):
        ...
        if self.domain == 'singlepoint':
            ...
            if self.stencil == 1:
                ...
                #return I,O 
                if self.troposphere_only:
                    return I[:,self.vi],O[:,self.vo]
                else:
                    return I,O
...
trainset1 = Dataset(files=train_files,domain='regional', region=rgn, stencil=1, manual_shuffle=False, batch_size=bs_train, troposphere_only=True)
...
testset1 = Dataset(files=test_files, domain='regional', region=rgn, stencil=1, manual_shuffle=False, batch_size=bs_test, troposphere_only=True)
...
file_prefix = f"torch_saved_models/regional_training/era5_regional_{sys.argv[1]}_troposphereonly_ann_cnn_leakyrelu_dropout0p2_cyclic_mseloss" 
